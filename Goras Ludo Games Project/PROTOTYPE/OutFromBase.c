...
void playerStartPoint() {//player-start point
	gotoxy(25,12);printf("%s%d",user.symbol,user.b[0]);
	gotoxy(34,12);printf("%s%d",user.symbol,user.b[1]);
	gotoxy(25,16);printf("%s%d",user.symbol,user.b[2]);
	gotoxy(34,16);printf("%s%d",user.symbol,user.b[3]);
...
void gamePlay() {
	...
	while (!out) {
		...
		if (r == 6) {
			gotoxy(29,21);printf("%s%d",user.symbol,user.b[0]);
			out = true;
		}
	}
	system("pause");
